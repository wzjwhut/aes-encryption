...
public class HexUtils {
    ...
    public static String dumpByteTable(int[][] table) {
        if(table == null){
            return "";
        }
        StringBuilder stringBuf = new StringBuilder(32);
        for(int[] row : table) {
            for (int bin : row) {
                stringBuf.append(chars[(bin >> 4) & 0x0f]);
                stringBuf.append(chars[bin & 0x0f]);
                stringBuf.append(' ');
            }
            stringBuf.append("\r\n");
        }
